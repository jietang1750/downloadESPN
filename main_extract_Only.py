...
def todaysMatchInDB(mysqlDict,eventList):
    ...
    try:
        ...
        (conn, cursor) = sqlConn.connectDB_ODBC(hostName, uid, pwd, dbName, odbcDriver)
...
def importEvents(bCompare, newFixtures,directory,dirSnapshots,mysqlImportDict,mysqlDateList):
    ...
    for tmpNewFixture in newFixtures:
        ...
        if tmpId in mysqlImportDict.keys() and bCompare:
            ...
            msg, bDiff = ESPNSoccer.compareFixtures3(tmpOldFixture, tmpNewFixture, endDateObj)
        else:
            bDiff = True
            strEvent="new event"
        ...
        if bDiff:
            if strEvent == "no change":
                ...
                diffList.append(tmpOldFixture)
                print(i, "of", len(newFixtures), tmpId,
                      tmpOldFixture['midSizeName'], strEvent,
                      tmpOldFixture['matchDateTime'],
                      tmpOldFixture['matchStatus'],
                      tmpNewFixture['date'],
                      tmpNewFixture['status'],
                      tmpNewFixture['name'])
...
if bImport:
    (newFixtures) = importFixtures(directory,league,league_list, importDates)
    tmpEventList = []
    ...
    (importedEventIds,importedTeamIds,importedLeagues,msg) = importEvents(bCompare,
                                                                          newFixtures,
                                                                          directory,
                                                                          dirSnapshots,
                                                                          mysqlImportDict,
    ...
    (teamsInLeagues, msg)=importStandings(directory,importedLeagues)
    print(msg)
    if bSaveInter:
...
msg = downloadLogos(teamLogoDir,sortedTeamList)
print(msg)
